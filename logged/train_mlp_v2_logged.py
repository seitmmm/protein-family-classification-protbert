...
def plot_confusion(
    ...
    y_pred,
    id_to_family,
    title="MLP v2 Confusion Matrix",
    normalize=True,
    save_path=None,
):
    """
    Рисует confusion matrix в «article style».

    normalize=True  -> нормализация по строкам (каждая строка суммируется в 1)
    save_path       -> если указан путь, сохраняем картинку вместо plt.show()
    """
...
def main():
    ...
    np.save(os.path.join(logs_dir, "mlp_v2_train_loss.npy"), np.array(train_losses))
    np.save(os.path.join(logs_dir, "mlp_v2_val_loss.npy"),   np.array(val_losses))
    np.save(os.path.join(logs_dir, "mlp_v2_train_acc.npy"),  np.array(train_accs))
    np.save(os.path.join(logs_dir, "mlp_v2_val_acc.npy"),    np.array(val_accs))
    print(f"Saved MLP v2 logs to {logs_dir}")
    ...
    print(f"\n=== MLP v2 TEST ACCURACY: {test_acc * 100:.2f}% ===")

    print("\nPer-class accuracy (MLP v2):")
    classes = sorted(list(set(y_true)))
    for cls in classes:
        mask = (y_true == cls)
    ...
    plot_confusion(
        ...
        y_pred,
        id_to_family,
        title="MLP v2 Confusion Matrix (normalized)",
        normalize=True,
        save_path=os.path.join(logs_dir, "mlp_v2_confusion_norm.png"),
    )

    # при желании ещё и «сырую» (ненормализованную) матрицу:
    plot_confusion(
        y_true,
        y_pred,
        id_to_family,
        title="MLP v2 Confusion Matrix (counts)",
        normalize=False,
        save_path=os.path.join(logs_dir, "mlp_v2_confusion_counts.png"),
    )
    ...
    title="MLP v2 Confusion Matrix",
    normalize=True,
    save_path=os.path.join(logs_dir, "mlp_v2_confusion_paper.png"),
)
