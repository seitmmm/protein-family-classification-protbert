...
class GaussianSplineActivation(nn.Module):
    def __init__(self, hidden_dim: int, num_knots: int = 5):
        ...
        self.log_scales = nn.Parameter(torch.zeros(hidden_dim, num_knots))
        self.coeffs = nn.Parameter(torch.randn(hidden_dim, num_knots) * 0.1)

    def forward(self, z):
        z_expanded = z.unsqueeze(-1)
        knots = self.knots.unsqueeze(0)
        scales = torch.exp(self.log_scales).unsqueeze(0) + 1e-6
        coeffs = self.coeffs.unsqueeze(0)

        diff = (z_expanded - knots) / scales
        gauss = torch.exp(-0.5 * diff * diff)
        out = (coeffs * gauss).sum(dim=-1)
        return out


class SmoothKANLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, num_knots: int = 5, dropout: float = 0.2):
        super().__init__()
        self.linear = nn.Linear(in_dim, out_dim)
        self.spline = GaussianSplineActivation(out_dim, num_knots=num_knots)
...
class SmoothKANClassifier(nn.Module):
    def __init__(self, input_dim: int, num_classes: int, num_knots: int = 5):
        super().__init__()
        self.layer1 = SmoothKANLayer(input_dim, 768, num_knots=num_knots, dropout=0.2)
        self.layer2 = SmoothKANLayer(768, 512, num_knots=num_knots, dropout=0.2)
        self.layer3 = SmoothKANLayer(512, 256, num_knots=num_knots, dropout=0.2)
...
def plot_confusion(
    ...
    y_pred,
    id_to_family,
    title="KAN (Gaussian) Confusion Matrix",
    normalize=True,
    save_path=None,
):
...
def main():
    ...
    model = SmoothKANClassifier(input_dim=input_dim, num_classes=num_classes, num_knots=5).to(device)
    ...
    np.save(os.path.join(logs_dir, "kan_smooth_train_loss.npy"), np.array(train_losses))
    np.save(os.path.join(logs_dir, "kan_smooth_val_loss.npy"),   np.array(val_losses))
    np.save(os.path.join(logs_dir, "kan_smooth_train_acc.npy"),  np.array(train_accs))
    np.save(os.path.join(logs_dir, "kan_smooth_val_acc.npy"),    np.array(val_accs))
    print(f"Saved Smooth KAN logs to {logs_dir}")
    ...
    print(f"\n=== Smooth KAN TEST ACCURACY: {test_acc * 100:.2f}% ===")

    print("\nPer-class accuracy (Smooth KAN):")
    classes = sorted(list(set(y_true)))
    for cls in classes:
        mask = (y_true == cls)
    ...
    plot_confusion(
        ...
        y_pred,
        id_to_family,
        title="KAN (Gaussian) Confusion Matrix (normalized)",
        normalize=True,
        save_path=os.path.join(logs_dir, "kan_gaussian_confusion_norm.png"),
    )
    ...
    plot_confusion_paper(
        ...
        title="KAN (Gaussian) Confusion Matrix",
        normalize=True,
        save_path=os.path.join(logs_dir, "kan_gaussian_confusion_paper.png"),
    )
